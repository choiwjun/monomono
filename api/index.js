    ...
    '<div class="traveler-header">' +
      ...
      '<div class="traveler-info">' +
        '<div class="traveler-name">' +
          ...
          user.name + ', ' + user.age +
          (user.verified ? '<span class="verified-badge">✓ 인증</span>' : '') +
        '</div>' +
    ...
    '<body>' +
      '<div class="app-container">' +
        ...
        '<nav class="bottom-nav">' +
          '<a href="/" class="nav-item ' + (activeNav === 'home' ? 'active' : '') + '"><span class="nav-icon"><i class="fas fa-home"></i></span>홈</a>' +
          '<a href="/travel" class="nav-item ' + (activeNav === 'travel' ? 'active' : '') + '"><span class="nav-icon"><i class="fas fa-plane"></i></span>여행</a>' +
          '<a href="/matches" class="nav-item ' + (activeNav === 'matches' ? 'active' : '') + '"><span class="nav-icon"><i class="fas fa-heart"></i></span>매칭</a>' +
          '<a href="/chat" class="nav-item ' + (activeNav === 'chat' ? 'active' : '') + '"><span class="nav-icon"><i class="fas fa-comment"></i></span>채팅</a>' +
...
function HomePage() {
  const heroSection = '<div class="hero">' +
    ...
    '<div class="hero-stats">' +
      '<div class="stat-item"><i class="fas fa-users"></i><span>1,247명 여행중</span></div>' +
      '<div class="stat-item"><i class="fas fa-plane"></i><span>서울 · 도쿄</span></div>' +
    '</div>' +
  '</div>';

  const categorySection = '<div class="section">' +
    '<div class="category-shortcuts">' +
      '<a href="/travel?type=companion" class="category-item"><div class="category-icon sakura"><i class="fas fa-user-friends"></i></div><span class="category-label">동행찾기</span></a>' +
      '<a href="/travel?type=guide" class="category-item"><div class="category-icon sky"><i class="fas fa-map-marker-alt"></i></div><span class="category-label">로컬가이드</span></a>' +
      '<a href="/travel?type=experience" class="category-item"><div class="category-icon mint"><i class="fas fa-star"></i></div><span class="category-label">체험</span></a>' +
      '<a href="/travel?type=event" class="category-item"><div class="category-icon sunset"><i class="fas fa-calendar-alt"></i></div><span class="category-label">이벤트</span></a>' +
    '</div>' +
  '</div>';
  ...
  return Layout('MONOMONO - 홈', 'home', heroSection + categorySection + travelersSection + safetyBanner);
}
...
function TravelPage() {
  const mySchedule = '<div class="section">' +
    '<div class="section-header">' +
      ...
      '<button class="btn btn-sakura" style="padding: 8px 16px; font-size: 12px;"><i class="fas fa-plus"></i> 일정 추가</button>' +
    '</div>' +
    '<div class="traveler-card" style="border: 2px solid var(--sakura);">' +
      ...
      '</div>' +
      '<p style="font-size: 13px; color: var(--gray-600); margin-bottom: 12px;">도쿄에서 한 달 살기! 라멘 맛집 탐방하고 현지 친구 사귀고 싶어요 🍜</p>' +
  ...
  return Layout('MONOMONO - 여행', 'travel', mySchedule + travelersSection);
}

function MatchesPage() {
  const newMatches = '<div class="section">' +
    '<div class="section-header"><h2 class="section-title">새로운 매칭</h2></div>' +
  ...
  const scheduleMatches = '<div class="section-header"><h2 class="section-title">여행 일정 매칭</h2></div>' +
    mockUsers.slice(0, 2).map(user => {
      return '<div class="traveler-card">' +
        '<div class="traveler-header">' +
          '<img src="' + user.photos[0] + '" alt="' + user.name + '" class="traveler-avatar">' +
          '<div class="traveler-info">' +
            '<div class="traveler-name"><span class="country-badge">' + getFlag(user.country) + '</span>' + user.name + ', ' + user.age + '<span class="verified-badge">✓ 인증</span></div>' +
            '<div class="traveler-meta">' + user.city + '</div>' +
          '</div>' +
        '</div>' +
        '<div class="travel-schedule">' +
  ...
  return Layout('MONOMONO - 매칭', 'matches', newMatches + scheduleMatches + safetyBanner);
}

function ChatPage() {
  const times = ['10분 전', '1시간 전', '어제'];
  const messages = ['안녕하세요! 서울에서 만나요 😊', '홍대 카페 추천해 주세요!', '다음에 또 연락해요~'];

  const chatList = '<div class="section">' +
    '<div class="section-header"><h2 class="section-title">채팅</h2></div>' +
  ...
  return Layout('MONOMONO - 채팅', 'chat', chatList);
}
    ...
    '<body>' +
      '<div class="app-container">' +
        '<header class="chat-header">' +
          ...
          '</div>' +
          '<div class="translation-toggle"><i class="fas fa-language"></i>자동 번역</div>' +
        '</header>' +
        '<div class="chat-messages">' +
          '<div class="message received">' +
            '<div class="message-bubble">' + (isJapan ? 'こんにちは！ソウル旅行楽しみにしています 😊' : '안녕하세요! 도쿄에서 만나요~') + '</div>' +
            '<div class="message-translation">' + (isJapan ? '안녕하세요! 서울 여행 기대하고 있어요 😊' : 'Hello! Let us meet in Tokyo~') + '</div>' +
            '<div class="message-time">오전 10:30</div>' +
          '</div>' +
          '<div class="message sent">' +
            '<div class="message-bubble">네! 저도 정말 기대돼요! 맛있는 곳 많이 알려주세요 🍜</div>' +
            '<div class="message-time">오전 10:32</div>' +
          '</div>' +
          '<div class="message received">' +
            '<div class="message-bubble">' + (isJapan ? 'もちろんです！美味しいラーメン屋さん知ってますよ' : '물론이죠! 맛있는 곳 많이 알아요') + '</div>' +
            '<div class="message-translation">' + (isJapan ? '물론이요! 맛있는 라멘집 알고 있어요' : 'Of course! I know many delicious places') + '</div>' +
...
function ProfilePage() {
  const content = '<div class="section">' +
    ...
    '<div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-bottom: 24px;">' +
      '<div class="traveler-card" style="text-align: center; padding: 16px 12px;"><div style="font-size: 24px; font-weight: 700; color: var(--sakura);">23</div><div style="font-size: 12px; color: var(--gray-400);">받은 좋아요</div></div>' +
      '<div class="traveler-card" style="text-align: center; padding: 16px 12px;"><div style="font-size: 24px; font-weight: 700; color: var(--sky);">12</div><div style="font-size: 12px; color: var(--gray-400);">매칭</div></div>' +
      '<div class="traveler-card" style="text-align: center; padding: 16px 12px;"><div style="font-size: 24px; font-weight: 700; color: var(--navy);">156</div><div style="font-size: 12px; color: var(--gray-400);">프로필 조회</div></div>' +
    '</div>' +
    '<div class="traveler-card">' +
      '<h3 style="font-weight: 600; margin-bottom: 12px;"><i class="fas fa-user" style="color: var(--sakura); margin-right: 8px;"></i>자기소개</h3>' +
      '<p style="font-size: 14px; line-height: 1.6; color: var(--gray-600);">도쿄에서 한 달 살기 예정입니다. 현지 친구 사귀고 싶어요! 라멘 맛집 추천해주세요 🍜</p>' +
    '</div>' +
    '<div class="traveler-card">' +
      '<h3 style="font-weight: 600; margin-bottom: 12px;"><i class="fas fa-heart" style="color: var(--sakura); margin-right: 8px;"></i>여행 스타일</h3>' +
      '<div class="traveler-tags"><span class="tag sakura">미식</span><span class="tag sakura">로컬체험</span><span class="tag sakura">힐링</span></div>' +
    '</div>' +
    '<div class="traveler-card">' +
      ...
      '<div style="display: flex; flex-direction: column; gap: 8px;">' +
        '<div style="display: flex; justify-content: space-between;"><span>🇰🇷 한국어</span><span style="color: var(--gray-400);">모국어</span></div>' +
        '<div style="display: flex; justify-content: space-between;"><span>🇯🇵 일본어</span><span style="color: var(--gray-400);">고급</span></div>' +
        '<div style="display: flex; justify-content: space-between;"><span>🇺🇸 영어</span><span style="color: var(--gray-400);">중급</span></div>' +
      '</div>' +
    '</div>' +
    '<div class="traveler-card" style="padding: 0;">' +
      '<a href="#" style="display: flex; align-items: center; justify-content: space-between; padding: 16px; border-bottom: 1px solid var(--gray-100); text-decoration: none; color: inherit;"><span><i class="fas fa-cog" style="color: var(--gray-400); margin-right: 12px;"></i>설정</span><i class="fas fa-chevron-right" style="color: var(--gray-400);"></i></a>' +
      '<a href="#" style="display: flex; align-items: center; justify-content: space-between; padding: 16px; border-bottom: 1px solid var(--gray-100); text-decoration: none; color: inherit;"><span><i class="fas fa-shield-alt" style="color: var(--gray-400); margin-right: 12px;"></i>안전 센터</span><i class="fas fa-chevron-right" style="color: var(--gray-400);"></i></a>' +
      '<a href="#" style="display: flex; align-items: center; justify-content: space-between; padding: 16px; text-decoration: none; color: inherit;"><span><i class="fas fa-question-circle" style="color: var(--gray-400); margin-right: 12px;"></i>도움말</span><i class="fas fa-chevron-right" style="color: var(--gray-400);"></i></a>' +
    '</div>' +
    '<button style="width: 100%; padding: 14px; margin-top: 16px; border: 1px solid var(--gray-200); border-radius: 12px; background: white; color: var(--gray-600); font-size: 14px; cursor: pointer;">로그아웃</button>' +
  '</div>';

  return Layout('MONOMONO - 프로필', 'profile', content);
}
    ...
    '<body>' +
      '<div class="app-container onboarding-container">' +
        '<div class="slide active" id="slide1">' +
          ...
          '<p class="slide-description">MONOMONO와 함께<br>새로운 여행 친구를 만나보세요!</p>' +
          '<div class="dots"><div class="dot active"></div><div class="dot"></div><div class="dot"></div></div>' +
          '<div class="onboarding-actions"><button class="btn btn-sakura" style="width: 100%;" onclick="showSlide(2)">다음</button></div>' +
        '</div>' +
        '<div class="slide" id="slide2">' +
          '<div class="slide-illustration sky">🗺️🎌</div>' +
          '<h1 class="slide-title">여행 일정으로<br>매칭하세요</h1>' +
          '<p class="slide-description">같은 시기에 여행하는 사람들과<br>자동으로 연결됩니다</p>' +
        ...
        '<div class="slide" id="slide3">' +
          ...
          '<p class="slide-description">실시간 번역으로 언어 장벽 없이<br>안전하게 대화하세요</p>' +
          ...
          '<div class="signup-form">' +
            '<div class="form-group">' +
              ...
              '<div class="country-select">' +
                '<div class="country-option selected" onclick="selectCountry(this)"><div class="flag">🇰🇷</div><div>한국</div></div>' +
                '<div class="country-option" onclick="selectCountry(this)"><div class="flag">🇯🇵</div><div>일본</div></div>' +
              '</div>' +
            '</div>' +
            '<div class="form-group"><label class="form-label">이메일</label><input type="email" class="form-input" placeholder="example@email.com"></div>' +
            '<div class="form-group"><label class="form-label">비밀번호</label><input type="password" class="form-input" placeholder="8자 이상 입력하세요"></div>' +
            '<div class="form-group"><label class="form-label">닉네임</label><input type="text" class="form-input" placeholder="여행 친구들에게 보여질 이름"></div>' +
          '</div>' +
          '<div class="onboarding-actions">' +
            '<button class="btn btn-sakura" style="width: 100%;" onclick="location.href=\'/\'">가입 완료</button>' +
            '<p style="font-size: 12px; color: var(--gray-400); text-align: center; margin-top: 12px;">가입 시 <a href="#" style="color: var(--sakura);">이용약관</a> 및 <a href="#" style="color: var(--sakura);">개인정보처리방침</a>에 동의합니다</p>' +
          '</div>' +
        '</div>' +
      '</div>' +
      '<script>' +
